...
\caption/f Abstracted representation of map into graph of nodes and edges.
\endinsert
...
\midinsert
\clabel[2_slowdown_features]{Example of slowdown features}
...
\midinsert
\clabel[3_feature_transfer]{Feature transfer}
...
\midinsert
\clabel[4_structure]{Common CNN structures}
...
Each node in supplied with longitude, latitude location, which gives us the option to enrich them with additional real world information. Figure \ref[5_yet_to_do]. shows the structure of initial data source.
	
\midinsert
\clabel[5_yet_to_do]{yet to do}
...
\midinsert
\clabel[6_bearing]{Bearing formula}
\picw=10cm \cinspic figures/3.2.2.1._formula_bearing.pdf 
\caption/f Bearing formula
\endinsert
...
\caption/f Google Street View API url generation
\endinsert
...
\midinsert
\clabel[7_edge_breakdown]{Breakdown of long edge}
\picw=13cm \cinspic figures/3.2.2.1._breakdown_of_edge.pdf 
\caption/f Breakdown of long edge into smaller sections
\endinsert
...
\midinsert
\clabel[8_ex_params]{Example of OSM data parameters}
\picw=13cm \cinspic figures/3.2.3._osm_map_with_params.pdf 
\caption/f Example of structure of OSM data with parameters
\endinsert

From implementation standpoint, we have downloaded the OSM data covering map of our location and loaded it into a PostgreSQL database. In this way we can send queries for lists of objects in the vicinity of queried location. We will get into more detail about implementation in \ref[ch6_osm_marker].
...
\midinsert
\clabel[9_ex_loc]{Different locations and their OSM vectors}
\picw=15cm \cinspic figures/3.2.3.1_locations_with_table.pdf 
\caption/f Example with different locations generating different OSM neighborhood vectors
\endinsert
...
\midinsert
\clabel[10b_six_imgs]{Generation of images and shared data}
\picw=10cm \cinspic figures/3.2.3.1_6images_and_their_shared_data.pdf 
\caption/f Generation of images and shared data
\endinsert
...
\caption/f Figure G. / Abstraction of model into feature extractor and classificator
\endinsert
...
\midinsert
\clabel[12_sigmoid_output]{Sigmoid activation formula}
\picw=13cm \cinspic figures/4.1.1._output_formula.pdf 
\caption/f Sigmoid activation formula
\endinsert
...
As was mentioned in \ref[ch2_feature_transfer] we reuse the model trained on large dataset of ImageNet, separate it from its classificatory and instead provide our own custom made top model. See Figure \ref[19_base_top].
...
We prevent the layers of base model from changing their weights and train only the newly attached top model for the new task. There are certain specifics connected with this approach which we will explore in \ref[ch4_specific_setting] section.
