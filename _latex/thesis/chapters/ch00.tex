...
\caption/f Formula describing a generic CNN architecture used with image data. Layers are used as building block for more complicated models. While designing a model, we need to keep in mind it's number of parameters.
\endinsert
...
\caption/f Sample of the structure of initial dataset. We are presented with a GeoJSON file with multiple properties stored for each Feature. Note that we are only interested in the location and score value in Edges file. 
\endinsert
...
\caption/f Illustration of Google Street View API url generation. We are standing in starting location and look in the direction calculated from the relative position of ending point. Note that Google Street View API also requires a private API key to allow for frequently repeated queries.
\endinsert
...
\caption/f Example of structure of OSM data with parameters showing its properties. Each object also has a location information and most cases also a non-trivial shape. We will be interested in getting the attribute-value pairs.
\endinsert
...
\captiontablehack/f Sample of interesting attributes and their possible values in OSM dataset. Note that there is a large number of attribute and value pairs that can be generated.
\endinsert
...
\captiontablehack/f Example of values given to a selection of objects from OSM dataset. Note that for some objects we can encounter empty values.
\endinsert
...
\caption/f The construction of neighborhood vector from collection of nearby objects. We need a list of objects in proximity of desired location and then count in their attribute-value pairs. Note that each pair has its own index reserved in the final vector. Only some of the pairs were selected as important, to limit the length of the OSM vector.
\endinsert
...
\caption/f An abstraction of CNN model into feature extractor section and classificator section. Feature extractor tries to abstract features from the concrete data and classificator projects the features back into concrete category or score data.
\endinsert
...
\caption/f Pooling layer structure is effectively downsampling the volume of input data. The operation applied when downsampling the data can be customized such as the maximal function in case of Max Pooling layer or average function in case of Average Pooling.
\endinsert
...
\caption/f Reusing part of already trained CNN model as a base model and adding our custom top model. Note that some of the features can be reused in between several runs of the code. We can change the character of the task from classification to regression problem by changing the custom top model.
\endinsert
...
\caption/f Example of differently sized images on the input which results in different feature vector size. While the architecture of the base model will adapt itself to arbitrarily sized input, we have to keep in mind the consequences this will have for the following top model.
\endinsert
...
\caption/f Final architecture of the Mixed model, which uses both OSM vector and the image data. There are several variables such as the depths and widths which control the shape of the final model as well as the number of it's parameters.
\endinsert
...
\caption/f Data augmentation example. Images can be mirrored or slightly slided in one or both of the axes. We should keep in mind the number of images we are generating in total to limit the complexity of resulting augmented dataset.
\endinsert
...
\caption/f Training stages illustrated for feature transfer approach. We are reusing values of already trained base model, while adjusting the weights of our custom model. Another stage of finetunning can be used to adjust even some weights of the original base model, however note that it usually is computationally expensive.
\endinsert
...
\caption/f Reusing saved image features from a file instead of costly computations, which saves time as well as the hardware requirements. We need to maintain the same order of the input data in relation with its labels, which requires a deterministic shuffling of the data. 
\endinsert
...
\caption/f Splitting schema employed in the k-fold cross-validation. We are given average score alongside with its standard deviation. The chosen value of "k" will influence the size of the validation data.
\endinsert
...
\caption/f Project structure in a schema. See the details for each of these boxes in this chapter.
\endinsert
...
\caption/t RunDownload code sample. It is the main function downloading new images from Google Street View service. We end up with a folder of images and generated Segments object saved in a pickled ".dump" file.
\endinsert
...
\endtt
\caption/t Exact Google Street View API url we need to generate. Note that we are controlling the dimensions of the image as well as location and the direction in which we are looking. We need a API key for repeating the requests frequently.
\endinsert
...
\caption/f Location outside of streets with available photographic imagery. We have to detect these images and filter them out.
\endinsert
...
images/5546_5.jpg
\endtt
\caption/t Folder structure of dataset. We keep the images downloaded in a separated folder with information of their location and labels in the saved Segments object in "SegmentsData.dump" file. Later we also add the downloaded OSM data into the stored Segments file.
\endinsert
...
\caption/t RunCheck code sample checks the downloaded dataset and redownloads missing images. As the downloading itself can take time and there is a daily limitation imposed by the Google Street View API, we want to fix the data on the fly.
\endinsert
...
\caption/t Loading data into PostgreSQL database. We are using the Osm2pgsql tool available at ~\cite[osm2pgsql]. 
\endinsert
...
\caption/t Marking data with OSM vector. We iterate over each Segment in Segments file marking its unique locations.
\endinsert
...
\caption/t Building generic model in Keras. This particular model will take input of any size and use Fully-connected layer followed by Dropout layer. The final output will be a single score per each input.
\endinsert
...
\caption/t Fit generic model to data in Keras and then save and plot the history of the training.
\endinsert
...
\caption/t Images only model using the feature cooking method. We can save the intermediate product of feature files to save on future computations.
\endinsert
...
\caption/t Default Settings initialization. Setup function is loaded from a customized setting file and alters the default values where it is needed.
\endinsert
...
\caption/f Settings syntax. Python dictionary object with list of models each also pointing to a dictionary. This structure allows for high degree of customization.
\endinsert
...
\caption/t Model training syntax. Model is trained on a specific dataset according to the settings. History is saved for later graph plotting.
\endinsert
...
\midinsert
\clabel[36_splitedge_2x2_img]{Split long edges, Image model, overall comparison}
\picw=13cm \cinspic graphs/6.1.2._splitting_long_edges-minlen/fig3_2x2comparison_img.pdf
\caption/f Model: \textit{Image}. Overall illustration of the training process of three datasets which were enhanced by long edge splitting and an original dataset for reference.
\endinsert

\midinsert
		\clabel[36_splitedge_valerr_img]{Split long edges, Image model, validation error over iterations}
		\picw=12cm \cinspic graphs/6.1.2._splitting_long_edges-minlen/fig2_average_valerr_img.pdf
		\caption/f Model: \textit{Image}, Datasets: \textit{original}, \textit{minlen10}, \textit{minlen20}, \textit{minlen30}. Long edge splitting experiment. For better clarity we present only the averages of validation errors over the 10 runs of k-fold cross-validation with k=10.
\endinsert

\midinsert
		\clabel[36_splitedge_boxplot_img]{Split long edges, Image model, comparison of best epoch between datasets}
		\picw=12cm \cinspic graphs/6.1.2._splitting_long_edges-minlen/fig1_boxplot_best_img.pdf
		\caption/f Model: \textit{Image}, Datasets: \textit{original}, \textit{minlen10}, \textit{minlen20}, \textit{minlen30}. Comparison of the results of splitting long edges into smaller segments. The smaller the minimal edge length was, the more images were generated. Dataset "5556x_markable_640x640" under the label original was used for comparison. Results are from the epoch with best validation error.
\endinsert

Starting with Image model, we chose to present the detailed information about the progress of training in Figure \ref[36_splitedge_2x2_img], which shows all four dataset variants alongside with its validation and training error. In Figure \ref[36_splitedge_valerr_img] we see the average validation errors plotted together in order to better compare their performances. Lastly in \ref[36_splitedge_boxplot_img] we can see the comparison of all four datasets with their best score over all epochs.

Note that we also tested the Mixed model in Appendix with Figures \ref[36_splitedge_valerr] and \ref[36_splitedge_boxplot].

When looking at Figure \ref[36_splitedge_boxplot_img] which portrays the comparison of Image model over the four datasets, we can see quite clearly that the expanded datasets perform better with their median and mean values always improved over the original dataset. In the case of “minlen30” dataset we can see vast improvement with the exception of several outliers. However note that for the Image model the validation error averages seem to converge to the same value in their last epoch as can be seen on Figure \ref[36_splitedge_valerr_img]. It is also worth noting, that in the initial 100 iterations the Expanded datasets performed better and only overfit towards the same mean average value with their 400th to 500th iteration. The best validation errors in Figure \ref[36_splitedge_boxplot_img] clearly come from these initial iterations.

For several further experiments we chose dataset “5556x\_minlen30\_640px” to accompany the original “5556x\_markable\_640x640”.

Situation in Figure \ref[36_splitedge_valerr] is different and the results show that the Mixed model is not as easily helped by the long edges splitting. Look at Figure \ref[36_splitedge_boxplot] shows the best result over all epochs and reveals that the altered datasets of “minlen10”, “minlen20” and “minlen30” didn’t bring much improvement over the original one. The spread of results in the non-original datasets is wider and seems to reach better score values, however both the median and mean of all expanded datasets has slightly worsened.
...
\line{\hsize=.5\hsize \vtop{%
      ...
      \picw=\prvniW \cinspic graphs/6.1.3._augmentation-original,expanded,agg_expanded/figLeft_best_markable.pdf
...
Figure \ref[37_augmentation] shows the evolution of validation error over the epochs, as well as the validation and training error distribution from k-fold cross-validation in its best epoch. On Figure \ref[37b_augmentation] we have the same experiment using "5556x_minlen30_640px" as the original dataset.

\midinsert
\clabel[37b_augmentation]{Result of augmented datasets with edge splitting}
\line{\hsize=.5\hsize \vtop{%
      %\clabel[37_augmentation_boxplots_last]{Dataset augmentation last epoch}
      \picw=\prvniW \cinspic graphs/6.1.3._augmentation-original,expanded,agg_expanded/figLeft_best_minlen30.pdf
      %\caption/f Last epoch validation error
   \par}\vtop{%
      %\clabel[37_augmentation_evol]{Dataset augmentation evolution over epochs}
...
\caption/f Model: \textit{OSM model width 64, depth 2}, Dataset: \textit{5556x\_markable\_640x640} from experiment \ref[ch6_model_osm_wd]. GeoJSON file visualization provided by the online service Carto.com. Note that all edges were evaluated as OSM model doesn’t depend on presence of imagery information. See this visualization online at \url{https://goo.gl/SdNKoC}.
\endinsert
